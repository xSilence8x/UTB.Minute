...
public sealed class KeycloakTokenProvider(IHttpClientFactory httpClientFactory, IConfiguration configuration, RoleState roleState)
{
    ...
    {
        LastError = null;
        var token = await RequestTokenAsync(username, password, cancellationToken);
        if (!string.IsNullOrWhiteSpace(token))
        {
            roleState.AccessToken = token;
        }
    ...
    {
        if (!string.IsNullOrWhiteSpace(roleState.AccessToken))
        {
            return roleState.AccessToken;
        }
